...
\end{wrapfigure}
A complex number can thus be identified with an ordered pair \((\Re(z), \Im(z))\) in the Cartesian plane, an identification sometimes known as the Cartesian form of \(z\). A complex number can be viewed as a point or position vector in a two-dimensional Cartesian coordinate system called the complex plane or Argand diagram, named after Jean-Robert Argand. The numbers are conventionally plotted using the real part as the horizontal component, and imaginary part as vertical (see Figure 1.1). In fact, a complex number can be defined as an ordered pair \((a, b)\), but then rules for addition and multiplication must also be included as part of the definition. William Rowan Hamilton introduced this approach to define the complex number system.
...
\begin{equation}
\begin{split}
	E_x = \int\displaylimits_{-\infty}^{+\infty} \abs{x(t)}^2 \dt = \sum_{k = -\infty}^{+\infty} \int\displaylimits_{-kT_0/2}^{kT_0/2} \abs{x(t)}^2 \dt =
	\quads{3}
	\\
	= \sum_{k = -\infty}^{+\infty} k \int\displaylimits_{-T_0/2}^{T_0/2} \abs{x(t)}^2 \dt = \sum_{k = -\infty}^{+\infty} k \cdot E_{T_0} = \lim_{k \rightarrow \infty} k \cdot E_{T_0} = \infty.
\end{split}
\end{equation}
Dove si \`e posto
\begin{equation}
	E_{T_0} = \int\displaylimits_{-T_0/2}^{T_0/2} \abs{x(t)}^2 \dt.
\end{equation}
...
\begin{split}
	...
	\\
	= \lim_{k \rightarrow \infty} \frac{1}{kT_0} \cdot k \cdot E_{T_0} = \lim_{k \rightarrow \infty} \frac{1}{T_0} \cdot E_{T_0} = \frac{1}{T_0} \int\displaylimits_{-T_0/2}^{T_0/2} \abs{x(t)}^2 \dt.
\end{split}
\end{equation}
\end{proof}
...
Ci\`o premesso, ci poniamo una domanda: qual \`e il modo pi\`u appropriato di procedere quando il segnale $x(t)$ \`e periodico con \textit{andamento arbitrario}, e in particolare \textit{non sinusoidali}? La risposta a questo quesito sta nella cosiddetta \textit{analisi di Fourier} che costituisce la base della moderna teoria dei segnali. Infatti, sotto ipotesi piuttosto ampie, che in seguito elencheremo, un segnale reale periodico qualunque pu\`o essere espresso come \textit{somma di oscillazioni sinusoidali di ampiezza, frequenza e fase opportune} cio\`e in una forma che richiami:
...
\begin{split}
	...
	\quads{2}
	\\
	= A_0 + \sum_{k = 1}^{\infty} A_k e^{j(2 \pi k f_0 t + \vartheta_k)} + e^{-j(2 \pi k f_0 t + \vartheta_k)} =
	\quads{2}
	\\
...
\]
\[
	= \frac{e^{j 2 \pi (k - n) f_0 t}}{j 2 \pi (k - n) f_0}\right]_{-T_0/2}^{T_0/2}
\]



Infine, l'espressione del generico coefficiente di Fourier $X_k$ \`e data da:
...
\begin{equation}
	P_x = \lim_{T \rightarrow \infty} P_{x_T} = \lim_{T \rightarrow \infty} \frac{E_{x_T}}{T} = \lim_{T \rightarrow \infty} \frac{K}{T} = 0,
\end{equation}
...
\end{split}
\end{equation}
Vediamo di risolvere questi tre integrali separatamente. Il primo integrale \`e ovvio
\begin{equation}
...
\end{equation}
\[
	= 2 \cdot \Re\left(x_m^* \cdot \lim_{T \rightarrow \infty} \frac{1}{T} \int\displaylimits_{-T/2}^{T/2} \tilde x(t) \dt\right) = 2 \cdot \Re\left(x_m^* \ \tilde x_m\right) =\footnote{Nella (4.19) abbiamo calcolato che $\tilde x_m = 0.$} \ 2 \cdot \Re\left(x_m^* \ 0\right) = 0.
\]
...
\newpage
\subsection{Biunivocit\`a della TSF}
...
\]
I due integrali sono risolvibili senza effettuare eccessivi calcolo matematici, infatti
\begin{equation}
	\cos(2 \pi (n - k) f_0 t) = 
		\begin{cases}
			1 \quad se \quad n = k\\
...
\begin{equation}
	E_u = \underbrace{\int\displaylimits_{-\infty}^{+\infty} \abs{u(t)}^2 \dt}_{u(t) \ = \ 0 \ \forall \ t \ < \ 0} = \underbrace{\int\displaylimits_{0}^{+\infty} 1 \dt}_{u(t) \ = \ 1 \ \forall \ t \ > \ 0} = \left[ t \right]_0^{+\infty} = +\infty - 0 = +\infty.
\end{equation}
La potenza media $P_u$, invece, \`e espressa dalla relazione
\begin{equation}
	\begin{split}
		P_u = \underbrace{\lim_{T \rightarrow \infty} \frac{1}{T} \int\displaylimits_{-T/2}^{T/2} \abs{u(t)}^2 \dt}_{u(t) \ = \ 0 \ \forall \ t \ < \ 0} = \underbrace{\lim_{T \rightarrow \infty} \frac{1}{T} \int\displaylimits_{0}^{T/2} \abs{u(t)}^2 \dt}_{u(t) \ = \ 1 \ \forall \ t \ > \ 0} =
		\\
		= \lim_{T \rightarrow \infty} \frac{1}{T} \left[ t \right]_0^{T/2} = \lim_{T \rightarrow \infty} \frac{1}{T} \left(\frac{T}{2} - 0\right) = \lim_{T \rightarrow \infty} \frac{1}{T} \cdot \frac{T}{2} = \frac{1}{2}.
	\end{split}
\end{equation}
