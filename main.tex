...
$Pr(A)$, espresso con il limite che abbiamo appena introdotto, \`e \textit{i})
una quantit\`a \textit{non negativa} poich\`e prodotta dal limite di un rapporto fra quantit\`a positive; se inoltre \textit{ii}) l'evento $A$ coincide con l'evento certo $\Omega$, allora banalmente $N_A = N$ e quindi $Pr(A) = 1$; se infine \textit{iii}) $A$ e $B$ sono due eventi mutuamente esclusivi, una prova dell'esperimento che fa verificare $A \cup B$ d\`a un risultato che sta o in $A$ o in $B$, ma che non pu\`o stare in entrambi, per cui $N_{A \cup B} = N_A + N_B$ e allora
\[
